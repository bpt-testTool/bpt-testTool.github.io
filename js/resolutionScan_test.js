...
//Global variables
let video_resolution_demo = $('#video_resolution_demo')[0], //where we will put & test our video output
...
$(document).ready(() => {
    ...
    navigator.mediaDevices.getUserMedia({ audio: false, video: true })
        .then((mediaStream) => {
            ...
            video_resolution_demo.srcObject = mediaStream;
...
function gum(candidate, device) {
    ...
    function gotStream(mediaStream) {
        ...
        video_resolution_demo.width = candidate.width;
        video_resolution_demo.height = candidate.height;
...
function displayVideoDimensions() {
    ...
    if (!video_resolution_demo.videoWidth) {
        setTimeout(displayVideoDimensions, 500); //was 500
    }

    if (video_resolution_demo.videoWidth * video_resolution_demo.videoHeight > 0) {
        if (tests[r].width + "x" + tests[r].height !== video_resolution_demo.videoWidth + "x" + video_resolution_demo.videoHeight) {
...
video_resolution_demo.onloadedmetadata = displayVideoDimensions;


//Save results to the candidate so
function captureResults(status) {
    console.log("Stream dimensions for " + tests[r].label + ": " + video_resolution_demo.videoWidth + "x" + video_resolution_demo.videoHeight);

    if (!scanning) //exit if scan is not active
        return;

    tests[r].status = status;
    tests[r].streamWidth = video_resolution_demo.videoWidth;
    tests[r].streamHeight = video_resolution_demo.videoHeight;
    ...
    if (r < tests.length) {
        ...
        video_resolution_demo.removeEventListener("onloadedmetadata", displayVideoDimensions); //turn off the event handler
